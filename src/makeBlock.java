...
public class makeBlock extends JFrame {
	block[][] block;	
	int px=10;	//block위치의 초기값(x좌표)
	int py=250;	//block위치의 초기값(y좌표)
	boolean blockBreak=true;	//block의 존재 유무
	boolean player=true;
	game game;
	int count1=55;	//block의 갯수
	int count2=55;	//block의 갯수
	makeBlock(game g){
		this.game=g;
		block=new block[10][11];	//행(11개) 열(10개)의 block
		//px값과 py값을 증가시켜주면서 block을 만든다.
		for(int x=0;x<10;x++) {
			for(int y=0;y<11;y++) {
