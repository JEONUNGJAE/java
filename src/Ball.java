...
public class Ball {
	...
	int x=500;	//공의 시작점 (x좌표)
	int y;	//공의 시작점 (y좌표)
	int vx=0;	//x가 한 번 이동할 거리
	int vy=5;	//y가 한 번 이동할 거리
	int player;
	...
	public Ball(game g,int i) {
		this.game=g;
		this.player=i;
		File input=new File("C:/Users/GG201305000004/eclipse-workspace/JavaProject/src/volleyball.png");	//Ball 이미지
		try {
		 ...
		 if(player==1) {
			 y=700;	//player1 공의 시작점(y좌표)
		 }else if(player==2) {
			 y=200;	//player2 공의 시작점(y좌표)
		 }
	}
	//Ball을 움직이는 함수
	public void move() {
